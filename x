...
local function drawBoundingFov(radius)
    -- Only draw if FovVisible is true
    if not settings.FovVisible then return end

    local halfSize = radius / 2
    local topLeft = Vector2.new(Mouse.X - halfSize, Mouse.Y - halfSize)
    local topRight = Vector2.new(Mouse.X + halfSize, Mouse.Y - halfSize)
    local bottomLeft = Vector2.new(Mouse.X - halfSize, Mouse.Y + halfSize)
    local bottomRight = Vector2.new(Mouse.X + halfSize, Mouse.Y + halfSize)
    ...
    -- Draw the bounding box (4 lines)
    table.insert(FovParts, createLine(topLeft, topRight))
    table.insert(FovParts, createLine(topRight, bottomRight))
    table.insert(FovParts, createLine(bottomRight, bottomLeft))
    table.insert(FovParts, createLine(bottomLeft, topLeft))
end

-- Helper function to draw a circle (if FovType is not boundingfov)
local function drawCircle(radius)
    -- Only draw if FovVisible is true
    if not settings.FovVisible then return end

    local circle = Drawing.new("Circle")
    circle.Visible = settings.FovVisible  -- Fov visibility controlled by setting
    ...
    if settings.Filled then
        circle.Transparency = settings.FillTransparency
    end
    table.insert(FovParts, circle)
end

-- Main function to update FOV based on the type
local function updateFov()
    local settings = getgenv().Silent.AimSettings.FovSettings
    clearFovParts()  -- Clear previous FOV parts

    if IsTargeting and TargetPlayer then  
        -- Get dynamic FOV radius based on the target distance
        local targetDistance = getDynamicFov(closestPoint)

        -- Check the Fov type and update accordingly
        if getgenv().Silent.Startup.Type == "boundingfov" then
            -- Draw bounding box if the type is "boundingfov"
            drawBoundingFov(targetDistance)
        else
            -- Default to Circle if the type is not "boundingfov"
            drawCircle(targetDistance)
        end
    end
end

-- Update FOV on each frame only while targeting
RunService.RenderStepped:Connect(function()
    if IsTargeting then
        updateFov()
    end
